...
// Copyright 2025 embeddedboys, Ltd.
...
// Subsample types
enum { JPEGE_SUBSAMPLE_444 = 0, JPEGE_SUBSAMPLE_420 };
...
// Compression quality
enum { JPEGE_Q_BEST = 0, JPEGE_Q_HIGH, JPEGE_Q_MED, JPEGE_Q_LOW };
...
typedef int32_t(JPEGE_READ_CALLBACK)(JPEGE_FILE *pFile, uint8_t *pBuf,
				     int32_t iLen);
typedef int32_t(JPEGE_WRITE_CALLBACK)(JPEGE_FILE *pFile, uint8_t *pBuf,
				      int32_t iLen);
...
typedef struct jpege_image_tag {
	...
	int iDCPred0, iDCPred1,
		iDCPred2; // DC predictor values for the 3 color components
	PIL_CODE pc;
	int *huffdc[2];
...
class JPEGENC {
    public:
	int open(const char *szFilename, JPEGE_OPEN_CALLBACK *pfnOpen,
		 JPEGE_CLOSE_CALLBACK *pfnClose, JPEGE_READ_CALLBACK *pfnRead,
		 JPEGE_WRITE_CALLBACK *pfnWrite, JPEGE_SEEK_CALLBACK *pfnSeek);
	int open(uint8_t *pOutput, int iBufferSize);
	int close();
	int encodeBegin(JPEGENCODE *pEncode, int iWidth, int iHeight,
			uint8_t ucPixelType, uint8_t ucSubSample,
			uint8_t ucQFactor);
...
int JPEGEncodeBegin(JPEGE_IMAGE *pJPEG, JPEGENCODE *pEncode, int iWidth,
		    int iHeight, uint8_t ucPixelType, uint8_t ucSubSample,
		    uint8_t ucQFactor);
int JPEGEncodeEnd(JPEGE_IMAGE *pJPEG);
int JPEGAddMCU(JPEGE_IMAGE *pJPEG, JPEGENCODE *pEncode, uint8_t *pPixels,
	       int iPitch);
int JPEGAddFrame(JPEGE_IMAGE *pJPEG, JPEGENCODE *pEncode, uint8_t *pPixels,
		 int iPitch);
