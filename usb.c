...
static void pud_fill_ctrl_buf(struct pud *pud, u16 xs, u16 ys, u16 xe, u16 ye,
			      size_t len)
{
	pud->ctrl_buf[0] = (xs & 0xff);
	pud->ctrl_buf[1] = (xs >> 8);
	pud->ctrl_buf[2] = (ys & 0xff);
	pud->ctrl_buf[3] = (ys >> 8);
	pud->ctrl_buf[4] = (xe & 0xff);
	pud->ctrl_buf[5] = (xe >> 8);
	pud->ctrl_buf[6] = (ye & 0xff);
	pud->ctrl_buf[7] = (ye >> 8);
	pud->ctrl_buf[8] = (len & 0xff);
	pud->ctrl_buf[9] = (len >> 8);
}
		  ...
		  size_t data_size)
{
	...
	pud_fill_ctrl_buf(pud, x, y, 480, 320, data_size);

	// request setup
	rc = usb_control_msg(udev, usb_sndctrlpipe(udev, EP0_OUT_ADDR),
			     REQ_EP1_OUT, TYPE_VENDOR | USB_DIR_OUT, 0, 0,
			     pud->ctrl_buf, sizeof(pud->ctrl_buf),
			     pud_DEFAULT_TIMEOUT);
				       ...
				       const struct usb_device_id *id)
{
	...
	pud->encoder_buf = devm_kmalloc(
		pud->dev, info->var.xres * info->var.yres * 2, GFP_KERNEL);
	if (!pud->encoder_buf)
		return -ENOMEM;
